...
def page_result():
    ...
    # **פרומפט מתוקן** למניעת סיכום ולהבטחת עברית בלבד
    summarize_prompt = f"""
    🔴 **הנחיות חשובות:**  
    - **כתוב את המשוב בעברית בלבד**.  
    - **אין לכתוב באנגלית. אם יש טקסט באנגלית – יש לתרגם אותו לעברית.**  
    - **אין לסכם את ההוראות. התחל מיד במשוב.**  
    - **עליך לתת משוב ישירות לסטודנט בגוף ראשון, ללא הקדמות מיותרות.**  

    📌 **פורמט מחייב של המשוב:**  
    **גילית אמפתיה כש...**  
    *[תיאור של דוגמה לאמפתיה שהפגין הסטודנט]*  

    **בדיקות קריטיות שביצעת:**  
    *[ציין אילו בדיקות קריטיות ביצע הסטודנט (סוכר, סטורציה, חום) ואילו החמיץ]*  

    **זיהוי היפוגליקמיה:**  
    *[פרט האם הסטודנט זיהה נכון את ההיפוגליקמיה ומה הציע כטיפול]*  

    **המלצות לשיפור:**  
    - *[המלצה ספציפית 1]*  
    - *[המלצה ספציפית 2]*  

    ⬇️ **הודעות הסטודנט בסימולציה:**  
    {student_text}
    """

    docs = [Document(page_content=summarize_prompt)]
    summarize_chain = load_summarize_chain(st.session_state.llm, chain_type="stuff")
    summary_response = summarize_chain.invoke(docs)['output_text']
